...
void copyPatch2(cv::Mat &img, int x, int y, uint8_t *patch_data) {
  for (int v = 0; v < g_patch_size; ++v, patch_data += g_patch_size) {
    uint8_t *img_ptr = img.data + (y - g_halfpatch_size + v) * img.cols +
                       (x - g_halfpatch_size);
...
void testZMSSD(cv::Mat &img) {
  ...
  // create patch
  uint8_t *ref_patch =
      vk::aligned_mem::aligned_alloc<uint8_t>(g_patch_size * g_patch_size, 16);
  uint8_t *cur_patch =
      vk::aligned_mem::aligned_alloc<uint8_t>(g_patch_size * g_patch_size, 16);
  ...
  int c = 10;
  uint8_t *data_ptr =
      img.data + (y - g_halfpatch_size) * img.cols + (x - g_halfpatch_size);
  for (int i = 0; i < 1000000; ++i) {
    PatchScore patch_score(ref_patch);
