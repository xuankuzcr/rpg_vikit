...
void halfSample(const cv::Mat &in, cv::Mat &out) {
  ...
  if (aligned_mem::is_aligned16(in.data) &&
      aligned_mem::is_aligned16(out.data) && ((in.cols % 16) == 0)) {
...
void calcSharrDeriv(const cv::Mat &src, cv::Mat &dst) {
  ...
  for (y = 0; y < rows; y++) {
    ...
    const uchar *srow2 = src.ptr<uchar>(y < rows - 1 ? y + 1
                                        : rows > 1   ? rows - 2
                                                     : 0);
    ...
    for (; x <= colsn - 8; x += 8) {
      ...
      // this can probably be replaced with aligned stores if we aligned dst
      // properly.
    ...
    for (; x < colsn; x++) {
      ...
      deriv_type t1 =
          (deriv_type)((trow1[x + cn] + trow1[x - cn]) * 3 + trow1[x] * 10);
      drow[x * 2] = t0;
      drow[x * 2 + 1] = t1;
    }
  }
...
#ifdef __SSE2__
void convertRawDepthImageSse_16u_to_32f(cv::Mat &depth_16u, cv::Mat &depth_32f,
                                        float scale) {
  ...
  for (int idx = 0; idx < depth_16u.size().area();
       idx += 8, input_ptr += 8, output_ptr += 8) {
    __m128 _input, mask;
