...
ATANCamera::ATANCamera(double width, double height, double fx, double fy,
                       double cx, double cy, double s)
    : AbstractCamera(width, height, 1.0), fx_(width * fx), fy_(height * fy),
      fx_inv_(1.0 / fx_), fy_inv_(1.0 / fy_), cx_(cx * width - 0.5),
      cy_(cy * height - 0.5), s_(s), s_inv_(1.0 / s_) {
  if (s_ != 0.0) {
...
Vector2d ATANCamera::world2cam(const Vector2d &uv) const {
  ...
  return Vector2d(cx_ + fx_ * dist_cam[0], cy_ + fy_ * dist_cam[1]);
}
