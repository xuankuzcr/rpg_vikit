...
OmniCamera::OmniCamera(string calibFile) {
...
Vector3d OmniCamera::cam2world(const double &u, const double &v) const {
  ...
  // Important: we exchange x and y since regular pinhole model is working with
  // x along the columns and y along the rows Davide's framework is doing
  // exactly the opposite

  double invdet = 1 / (ocamModel.c - ocamModel.d * ocamModel.e);

  xyz[0] = invdet * ((v - ocamModel.xc) - ocamModel.d * (u - ocamModel.yc));
  xyz[1] = invdet * (-ocamModel.e * (v - ocamModel.xc) +
                     ocamModel.c * (u - ocamModel.yc));

  double r = sqrt(
      pow(xyz[0], 2) +
      pow(xyz[1], 2)); // distance [pixels] of  the point from the image center
...
Vector2d OmniCamera::world2cam(const Vector3d &xyz_c) const {
  ...
  double norm =
      sqrt(pow(worldCoordinates_bis[0], 2) + pow(worldCoordinates_bis[1], 2));
  double theta = atan(worldCoordinates_bis[2] / norm);

  // Important: we exchange x and y since Pirmin's stuff is working with x along
  // the columns and y along the rows, Davide's framework is doing exactly the
  // opposite
  double rho;
  double t_i;
  double x;
