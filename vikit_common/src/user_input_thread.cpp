...
UserInputThread::UserInputThread() : stop_(false), input_((char)0) {
  tcgetattr(0, &original_terminal_settings_); // save old terminal i/o settings
  new_terminal_settings_ =
      original_terminal_settings_; // make new settings same as old settings
  ...
  tcsetattr(0, TCSANOW,
            &new_terminal_settings_); // use these new terminal i/o settings now

  user_input_thread_ =
      new std::thread(&UserInputThread::acquireUserInput, this);
}
...
void UserInputThread::stop() { stop_ = true; }

void UserInputThread::acquireUserInput() {
  int c = 0;
  while (!stop_) {
    c = getchar(); // TODO: this is blocking, so the interruption point is not
                   // reached...
    if ((char)c == ' ')
