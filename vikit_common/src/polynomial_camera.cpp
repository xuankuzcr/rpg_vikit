...
//               double k2, double k3, double k4, double k5, double k6, double
//               k7) : AbstractCamera(width * scale , height * scale, scale),
//               fx_(fx * scale), fy_(fy * scale), cx_(cx * scale), cy_(cy *
//               scale), skew_(skew * scale), distortion_(fabs(k2) > 0.0000001)
// {
//   cout << "scale: " << scale << endl;
//   k2_ = k2; k3_ = k3; k4_ = k4; k5_ = k5; k6_ = k6; k7_ = k7;
// }

PolynomialCamera::PolynomialCamera(double width, double height, // double scale,
                                   double fx, double fy, double cx, double cy,
                                   double skew, double k2, double k3, double k4,
                                   double k5, double k6, double k7)
    : AbstractCamera(width, height, 1.0), fx_(fx), fy_(fy), cx_(cx), cy_(cy),
      skew_(skew), distortion_(fabs(k2) > 0.0000001) {
  // cout << "scale: " << scale << endl;
  k2_ = k2;
  k3_ = k3;
  k4_ = k4;
...
Vector3d PolynomialCamera::cam2world(const double &u, const double &v) const {
  Vector3d xyz;
  if (!distortion_) {
    // xyz[0] = (u - cx_)/fx_;
    // xyz[1] = (v - cy_)/fy_;
...
Vector3d PolynomialCamera::cam2world(const Vector2d &uv) const {
  return cam2world(uv[0], uv[1]);
}

Vector2d PolynomialCamera::world2cam(const Vector3d &xyz) const {
...
Vector2d PolynomialCamera::world2cam(const Vector2d &uv) const {
  Vector2d px;
  if (!distortion_) {
    px[0] = fx_ * uv[0] + cx_;
    px[1] = fy_ * uv[1] + cy_;
