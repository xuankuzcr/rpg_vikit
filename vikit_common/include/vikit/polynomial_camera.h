...
private:
  ...
  const double skew_;
  bool distortion_; //!< is it pure pinhole model or has it equidistant
                    //!< distortion
...
public:
  ...
  //               double k2=0.0, double k3=0.0, double k4=0.0, double k5=0.0,
  //               double k6=0.0, double k7=0.0);
  ...
  inline double thetad_from_theta(const double theta) const {
    ...
    const double thetad = theta + k2_ * theta2 + k3_ * theta3 + k4_ * theta4 +
                          k5_ * theta5 + k6_ * theta6 + k7_ * theta7;
    return thetad;
  }
  ...
  inline double deriv_thetad_from_theta(const double theta) const {
    ...
    const double theta5 = theta4 * theta;
    const double theta6 = theta5 * theta;
    return 1 + 2 * k2_ * theta + 3 * k3_ * theta2 + 4 * k4_ * theta3 +
           5 * k5_ * theta4 + 6 * k6_ * theta5 + 7 * k7_ * theta6;
  }
  ...
  jacobian_2x3(const Eigen::Vector3d &p) const {
