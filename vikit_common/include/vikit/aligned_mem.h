...
template <class T, int N = 20> struct placement_delete {
  ...
  static inline void destruct(T *buf) { (*(Array *)buf).~Array(); }
...
inline void aligned_free(void *memory) { free(memory); }

template <class T> inline T *aligned_alloc(size_t count, size_t alignment) {
  void *data = aligned_alloc(sizeof(T) * count, alignment);
  return new (data) T[count];
}

template <class T> inline void aligned_free(T *memory, size_t count) {
...
template <class T, int N> struct AlignedMem {
  T *mem;
  size_t count;
  AlignedMem(size_t c) : count(c) { mem = aligned_alloc<T>(count, N); }
  ~AlignedMem() { aligned_free<T>(mem, count); }
  T *data() { return mem; }
