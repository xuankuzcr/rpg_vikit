...
#ifndef VIKIT_NLLS_SOLVER_H_
#define VIKIT_NLLS_SOLVER_H_
...
template <int D, typename T> class NLLSSolver {
...
protected:
  ...
  ModelType prior_;
  Matrix<double, D, D>
      I_prior_; //!< Prior information matrix (inverse covariance)
  double chi2_;
  double rho_;
  Method method_;
  ...
  virtual double computeResiduals(const ModelType &model, bool linearize_system,
...
public:
  /// Damping parameter. If mu > 0, coefficient matrix is positive definite,
  /// this ensures that x is a descent direction. If mu is large, x is a short
  /// step in the steepest direction. This is good if the current iterate is far
  /// from the solution. If mu is small, LM approximates gauss newton iteration
  /// and we have (almost) quadratic convergence in the final stages.
  double mu_init_, mu_;
  ...
  NLLSSolver()
      : have_prior_(false), method_(LevenbergMarquardt), mu_init_(0.01f),
        mu_(mu_init_), nu_init_(2.0), nu_(nu_init_), n_iter_init_(15),
        n_iter_(n_iter_init_), n_trials_(0), n_trials_max_(5), n_meas_(0),
        stop_(false), verbose_(true), eps_(0.0000000001), iter_(0),
        use_weights_(false), scale_(0.0), scale_estimator_(NULL),
  ...
  /// Specify the robust cost that should be used and the appropriate scale
  /// estimator
  void setRobustCostFunction(ScaleEstimatorType scale_estimator,
                             WeightFunctionType weight_function);

  /// Add prior to optimization.
  void setPrior(const ModelType &prior,
